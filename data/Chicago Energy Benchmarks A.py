...
for i in range(len(data)):
    try:
        built = data[i][headers.index("Year Built")]
        electrical = float(data[i][headers.index("Total GHG Emissions (Metric Tons CO2e)")])
        square_ft = float(data[i][headers.index("Gross Floor Area - Buildings (sq ft)")])
        name = data[i][2]
